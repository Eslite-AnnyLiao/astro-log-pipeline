...
// 統計 Googlebot 對「同一分類頁」的重爬間隔
// 資料源：category SSR log；頁面 ID = L1/L2/L3 組合鍵

const fs = require('fs');
const path = require('path');
const { parse } = require('csv-parse');

const DIR = '/Users/liaoliting/Webserver/astro-log-pipeline/to-analyze-daily-data/category/ssr';
const START = process.argv[2] || '20260802';
const END = process.argv[3] || '20260831';
const MIN_GAP_MS = Number(process.argv[4] || 60000);
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(fp)
      ...
      .on('data', (row) => {
        ...
        if (!arr) { arr = []; timestamps.set(key, arr); }
        arr.push(t);
      })
      .on('end', () => {
...
(async () => {
  const files = fs.readdirSync(DIR)
    .filter((f) => /^category-page-log-\d{8}\.csv$/.test(f))
    .filter((f) => {
  ...
  process.stderr.write(`Files: ${files.length} (${START}~${END}), MIN_GAP_MS=${MIN_GAP_MS}\n`);
  const t0 = Date.now();
  ...
  const summary = {
    period: `${START}~${END}`,
    days: files.length,
    filter: `UA 含 Googlebot/2.1; MIN_GAP_MS=${MIN_GAP_MS}`,
    dataset: 'category SSR render log（Datadog）',
    pageIdBasis: 'L1/L2/L3 組合鍵',
    ...
    hitsPerPage: hitsPerPage.length ? {
